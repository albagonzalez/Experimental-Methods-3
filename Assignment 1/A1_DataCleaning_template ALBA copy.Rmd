...
setwd("/Users/al/RStudio/Semester 3/Experimental-Methods-3/Assignment 1")
```
...
```{r}
#Summary to overview what columns don't match
summary(demo_train)
summary(LU_train)
summary(token_train)

#Renaming the columns for the different dataframes to match
demo_train <- dplyr::rename(demo_train, c("VISIT" = "Visit"))
demo_train <- dplyr::rename(demo_train, c("SUBJ" = "Child.ID"))
```
...
```{r}
library("stringr")
#Changing the data inputs on LU train and token train dataframes to more simple ones (instead of Visit 1, visit 2... we wrote just 1,2,3...)
...
```{r}
#Removing punctuation from thw column Subject
demo_train$SUBJ <- stringr::str_replace_all(demo_train$SUBJ, "[[:punct:]]", "")
LU_train$SUBJ <- stringr::str_replace_all(LU_train$SUBJ, "[[:punct:]]", "")
token_train$SUBJ <- stringr::str_replace_all(token_train$SUBJ, "[[:punct:]]", "")
```
...
```{r}
#Select the columns that we want to use from each dataframe
demo_train2 <- select(demo_train, "VISIT", "SUBJ", "Diagnosis", "Ethnicity", "Gender", "Age", "ADOS", "MullenRaw", "ExpressiveLangRaw", "Socialization")
LU_train2 <- select(LU_train, "VISIT", "SUBJ", "MOT_MLU", "CHI_MLU")
token_train2 <- select(token_train, "VISIT", "SUBJ", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")
```
...
```{r}
            ### MY VERSION ###
#Creating a subset for only the variables ADOS, MullenRaw, ExpressiveLangRaw and Socialization for visit 1.
subset1 <- select(df2, SUBJ, VISIT, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
#Filtering out the visits that are not the 1st one
subset1 <-filter(subset1, VISIT == "1")
#Deleting the VISIT column
subset1 <- select(subset1, -VISIT)

#Renaming the clinical variables
subset1 <- dplyr::rename(subset1, c("ADOS1" = "ADOS"))
subset1 <- dplyr::rename(subset1, c("ExpressiveLangRaw1" = "ExpressiveLangRaw"))
subset1 <- dplyr::rename(subset1, c("MullenRaw1"="MullenRaw"))
subset1 <- dplyr::rename(subset1, c("Socialization1" = "Socialization"))

#Merge by subject so that the merged subjects are copied on every session/visit
df2 <- merge(df2, subset1, by = "SUBJ")
...
df2$ADOS1 <- df2$ADOS
df2$MullenRaw1 <- df2$MullenRaw
df2$ExpressiveLangRaw1 <- df2$ExpressiveLangRaw
df2$Socialization1 <- df2$Socialization

#Putting NAs where the visits 2-6
df2$ADOS1 <- ifelse(df2$VISIT %in% c("2", "3", "4", "5", "6"), NA, df2$ADOS1)
df2$MullenRaw1 <- ifelse(df2$VISIT %in% c("2", "3", "4", "5", "6"), NA, df2$MullenRaw1)
df2$ExpressiveLangRaw1 <- ifelse(df2$VISIT %in% c("2", "3", "4", "5", "6"), NA, df2$ExpressiveLangRaw1)
df2$Socialization1 <- ifelse(df2$VISIT %in% c("2", "3", "4", "5", "6"), NA, df2$Socialization1)

# Just making a little new data frame to get an overview of what we did.
new <- select(df2, SUBJ, VISIT, ADOS1, MullenRaw1, ExpressiveLangRaw1, Socialization1)
...
df2$SUBJ <- as.numeric(df2$SUBJ)
df2$SUBJ <- as.character(df2$SUBJ)

#Renaming the levels of the factor gender and diagnosis
# Renaming 1 and 2 in the gender column to M and F:
df2$Gender <- gsub("1", "M", df2$Gender)
df2$Gender <- gsub("2", "F", df2$Gender)

#Renaming the levels of the factor Diagnosis from A and B to ASD and TD:
df2$Diagnosis <- gsub("A", "ASD", df2$Diagnosis)
df2$Diagnosis <- gsub("B", "TD", df2$Diagnosis)
